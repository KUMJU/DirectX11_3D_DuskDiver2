...
HRESULT CGraphicDev::ReadyDepthStencilRenderTargetView(_uint iWinCX, _uint iWinCY)
{
	if (nullptr == m_pDevice)
		return E_FAIL;
...
ULONG CGraphicDev::GetRefCount(const wrl::ComPtr<ID3D11Device>& p)
{
	ID3D11Device* temp = p.Get();
...
std::shared_ptr<CGraphicDev> CGraphicDev::Create(const GRAPHIC_DESC& GraphicDesc)
{
	shared_ptr<CGraphicDev> pInstance = make_shared<CGraphicDev>();

	if (FAILED(pInstance->ReadyGraphicDevice(GraphicDesc)))
	{
